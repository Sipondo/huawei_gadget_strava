...
def analyze_workout(workout_dir: Path, output_dir: Path) -> Path:
    ...
    output_fname = output_dir / f"{workout_id}_indoor_running.fit"
    ...
    if not data_fname.exists() or not summary_fname.exists():
        raise FileNotFoundError(
            "Missing indoor running files. Expected HUAWEI_WORKOUT_DATA_SAMPLE.csv "
            "and HUAWEI_WORKOUT_SUMMARY_SAMPLE.csv in the workout folder."
        )

    print("Reading indoor running data files...")
    ...
    for column_name in required_columns:
        if column_name not in df_data.columns:
            raise ValueError(f"Missing required running data column: {column_name}")
    ...
    if df_data.empty:
        raise ValueError("No valid heart-rate records found for indoor running workout.")
    ...
    if hasattr(Sport, "RUNNING"):
        session.sport = Sport.RUNNING
    else:
        session.sport = cast(Sport, 1)

    if hasattr(SubSport, "INDOOR_RUNNING"):
        session.sub_sport = SubSport.INDOOR_RUNNING
    elif hasattr(SubSport, "TREADMILL"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze an indoor running workout folder.")
